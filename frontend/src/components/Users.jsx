...
export default function Users() {
  ...
  return (
    <div className='mt-5 space-y-2'>
      ...
      <input value={filter} onChange={(e)=> setFilter(e.target.value)}  className='w-full p-1 px-2 mb-5 bg-gray-200 border border-gray-500 rounded-md' type="text" placeholder='Search users...' />
      {users.map((item)=>(
        ...
        <div>
            <button onClick={()=> {navigate(`/send?id=${item._id}&name=${item.firstName}`)}} className='p-1 mt-5 text-white bg-black rounded-md md:text-xl'>Send money</button>
        </div>
      </div>
      ))} 
    </div>
